...
if expressed_genes_file:
    enr_stats = enr_stats.groupby(['dataset', 'motif', 'real_int', 'shuffled_int', 'p_val', 'enr_fold', 'adj_pval', 'pi_value', 'rank_pi_val', 'family', 'Any expressed gene']).agg(lambda x: ','.join((x.dropna()))).reset_index().sort_values(by = 'rank_pi_val')
    
if not expressed_genes_file:
    enr_stats = enr_stats.groupby(['dataset', 'motif', 'real_int', 'shuffled_int', 'p_val', 'enr_fold', 'adj_pval', 'pi_value', 'rank_pi_val', 'family']).agg(lambda x: ','.join((x.dropna()))).reset_index().sort_values(by = 'rank_pi_val')


enr_stats = enr_stats.rename(columns = {'dataset': 'Dataset name', 'gene_id': 'Gene ID', 'TF_rank': 'TF rank', 'gene_name': 'Gene name', 'description': 'Gene description', 'family': 'TF family', 'ath_gene_id': 'Arabidopsis gene ID', 'ath_alias': 'Arabidopsis gene name', 'gene_name_figs': 'Gene name (+ alias)', 'motif': 'Motif IDs', 'enr_fold': 'Motif enrichement fold', 'p_val': 'Motif enrichment p-value', 'adj_pval': 'Motif enrichment q-value', 'pi_value': 'Motif enrichment pi-value', 'rank_pi_val': 'Motif rank (pi-value)', 'real_int': 'Real motif overlaps', 'shuffled_int': 'Shuffled motif overlaps'})
