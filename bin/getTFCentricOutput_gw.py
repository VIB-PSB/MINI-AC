...
enr_stats = enr_stats.groupby('gene_id').agg({'dataset': 'first', 'motif_min_rank': 'first', 'min_motif_rank': min, 'min_qval': min, 'motif': ','.join,'enr_fold': ','.join, 'adj_pval': ','.join,'rank_pi_val': ','.join}).sort_values(by = 'min_motif_rank').reset_index()
...
if DE_table_file:
    ...
    enr_stats = enr_stats[['dataset', 'gene_id',  'motif_min_rank', 'min_motif_rank', 'min_qval', 'family'] + info_df_columns + DE_table_columns + net_cols + ['motif', 'enr_fold', 'adj_pval', 'rank_pi_val']]
